...
\begin{frame}
    ...
    Assuming that the input voltage is an \hl{ideal sinusoidal signal} $$u_1(t) = \hat{u}_1 \sin(\omega t)$$ with the angular frequency $\omega = 2\pi f$ and the amplitude $\hat{u}_1$, the task of a rectifier is to convert this input into a \hl{unidirectional, ideally constant, voltage} $u_2(t)\approx u_2$, as shown in  \figref{fig:rectification_task}. A typical application is the grid voltage rectification in power supplies.
...
\begin{frame}
    ...
    \end{equation}
    Here, it should be noted that both $u_\mathrm{s,1}(t)$ and $u_\mathrm{s,2}(t)$ are utilizing only half of the secondary winding turns due to the central tapping.\pause The \hl{output voltage} results in
    \begin{equation}
...
\begin{frame}
    \frametitle{B2U rectifier with capacitive output filtering and grid impedance (cont.)}
    \onslide<1->{In periodic \hl{steady-state DCM} the output current is zero for
   \begin{equation}
        i_2(\omega t) = 0, \quad 0 \leq \omega t < \omega t_1.
   \end{equation}
   Until then the diode bridge is in blocking mode and disconnects the input from the output.}\onslide<2->{ \hl{At $\omega t_1 = \alpha$ the diodes start conducting} since the input voltage exceeds the output voltage:}
   \begin{equation}
...
\begin{frame}
    \frametitle{B2U rectifier with capacitive output filtering and grid impedance (cont.)}
    \onslide<1->{\hl{At $\omega t_3 = \alpha + \beta$ the current reaches zero again} and the diode bridge blocks again:}
   \begin{equation}
    \begin{split}
        \onslide<1->{i_2(\omega t_3) &= \frac{\hat{u}_1}{\omega L} \left(\cos(\alpha)-\cos(\omega t_3)-\frac{U_2}{\hat{u}_1}(\omega t_3 - \alpha)\right) \stackrel{!}{=} 0}\\
...
\begin{frame}
    ...
    \begin{equation}
        ...
        \end{cases}
    \end{equation}}%
    \onslide<2->{The minus sign during the second half-cycle results from the conducting diodes D3/D4 reversing the current direction in the inductor -- cf. \figref{fig:B2U_topology_capacitive_filter_grid}.}\onslide<3->{ The input current can be decomposed via the Fourier series into its \hl{fundamental and harmonic components}:
    \begin{equation}
    ...
    \end{equation}}%
    \onslide<4->{As will be discussed in the following, the \hl{harmonic components $i_1^{(\mathrm{h})}(t)$ are considered distortions} negatively impacting the grid quality.}
\end{frame}
...
\begin{frame}
    ...
    \end{equation}
    with $\overline{u}$ and $\overline{i}$ being the DC components, $\hat{u}^{(k)}$ and $\hat{i}^{(k)}$ the amplitudes of the $k$-th harmonic and $\varphi_u^{(k)}$ and $\varphi_i^{(k)}$ the phase angles of the $k$-th voltage and current harmonics.\pause This amplitude-phase representation is analogous to the orthogonal representation \eqref{eq:Fourier_series} with the relations:
    \begin{equation}
...
\begin{frame}
    ...
    \onslide<1->{The duty cycle from \eqref{eq:PFC_duty_cycle_feed_forward} does not consider the \hl{inner voltage demand} of the boost converter, in particular of its filter inductor $L$:} 
    \begin{equation}
    \begin{split}
...
\begin{frame}
    ...
    \end{equation}
    \onslide<4->{If the load current $I_0$ is (approximately) constant, the \hl{harmonic current is flowing into the filter capacitor} $i_2^{(\mathrm{h})}(t)=i_\mathrm{C}(t)$}\onslide<5->{ leading to the voltage ripple:
    \begin{equation}
...
\begin{frame}
    ...
    \end{itemize}}
    \onslide<3->{The \hl{average output voltage} can be found by evaluating the conduction interval of one phase, e.g., $u_{1\mathrm{a}}(t)$:}
    \begin{equation}
        \begin{split}
...
\begin{frame}
    \frametitle{M3U rectifier with output filter (cont.)}
    \onslide<1->{The \hl{average inductor current} can be calculated as}
    \begin{equation}
        \begin{split}
    ...
    \begin{equation}
        \begin{split}
            \onslide<4->{\overline{i}_2 &= \frac{\hat{u}_1}{\omega L}\left[\cos(\omega t_1) + \sin(\omega t_1)(\omega t_1 - \frac{\pi}{2})\right]\\} \onslide<5->{
                           & =  \frac{\overline{u}_2}{\omega L}\left[\cot(\omega t_1) + \omega t_1 - \frac{\pi}{2}\right].}
        \end{split}
    \end{equation}
    \onslide<6->{One can also reinterpret this result for designing the filter inductor $L$ to ensure CCM operation:
    \begin{equation}
        L \geq \frac{\overline{u}_2}{\omega \overline{i}_2}\left[\cot(\omega t_1) + \omega t_1 - \frac{\pi}{2}\right].
    \end{equation}}
\end{frame}
